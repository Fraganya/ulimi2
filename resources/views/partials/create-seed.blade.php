<div class="modal fade" id="create-seed-window" data-backdrop="static">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4 class="modal-title">New Seed</h4>
            </div>
            <div class="modal-body">
                <form action="{{url('create-seed')}}" method="POST" id="seed-form" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group {{($errors->has('seed_category_id')) ? 'has-error' : ''}}">
                        <label for="">Category</label>
                        <select name="seed_category_id"  class="form-control">
                            @foreach($seed_categories as $category)
                                <option value="{{$category->id}}">{{$category->name}}</option>
                            @endforeach
                        </select>
                        @if($errors->has('seed_category_id'))
                            <span class="help-block">
                                {{$errors->first('seed_category_id')}}
                            </span>
                        @endif
                    </div>
                    ...
                    <div class="form-group {{($errors->has('sample_picture')) ? 'has-error' : ''}}">
                        <label for="">Seed Sample Image</label>
            ...
            <div class="modal-footer">
                <div class="pull-left">
                    <button onclick="document.getElementById('seed-form').submit()" class="btn btn-primary">Save Seed</button>
                </div>
