...
class FeedCatController extends Controller
{
    /**
    ...
    {
        return view('feed-categories')->with([
            'categories'=>FeedCat::simplePaginate(15),
            'c_page'=>"feedcats"
        ]);
    }
    ...
    public function store(Request $request)
    {
        if($request->input('web_access')){
            \Session::flash('target-window',"#create-feed-cat-window");
            ...
            if(FeedCat::where('name',$request->name)->first()){
        ...
        return json_encode(['status'=>'true','message'=>"Seed Save successful",'user'=>$category]);
    }

    public function removeCat($catID)
    {
        $cat=FeedCat::findOrFail($catID);
